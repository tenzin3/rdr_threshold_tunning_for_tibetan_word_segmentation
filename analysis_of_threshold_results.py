...
from src.word_segmentation_rules_generator.eval_rdr_result.eval_rdr_result import eval_rdr_result_and_return_wrong_tagged
...
for threshold in THREHOLD_LIST:
    ...
    training_wrong_tagged_wordtag, training_accuracy = eval_rdr_result_and_return_wrong_tagged()
    testing_wrong_tagged_wordtag, testing_accuracy = eval_rdr_result_and_return_wrong_tagged('TIB_test_maxmatched_tagged.txt', 'TIB_test_maxmatched.txt.TAGGED')
    
    with open('analysis_result.txt', 'w', encoding='utf-8') as file:
        file.write('Training Errors-----> \n')
        file.write(training_wrong_tagged_wordtag)
        file.write('Testing Errors-----> \n')
        file.write(testing_wrong_tagged_wordtag)
